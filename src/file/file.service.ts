...
export class FileService {
  ...
  // дальше принимает папку и по дефолту ставим products потому что у нас загрузка файлов будет только для продуктов
  async saveFiles(files: Express.Multer.File[], folder: string = 'products') {
    ...
    const response: FileResponse[] = await Promise.all(
      ...
      // в маp используеться async потому что запись идёт асинхронно (через промис), и вы ждёте, пока файл полностью запишется на диск, прежде чем идти дальше.
