...
export const useCreateStore = () => {
  ...
  // производим мутацию
  // используем useMutation только с Идемпотентные запросами, а useQuery только с GET
  // POST
  // PUT
  // DELETE
  ...
  const { mutate: createStore, isPending: isLoadingCreateStore } = useMutation({
    ...
    onSuccess(store) {
      ...
      // т.е в onSuccess нам приходит ответ и мы отправляем запрос и смотрим что за данные у юзера
      // чтобы сразу получать обновленые данные и сразу их отображать на ui
      // Реинвалидация будет срабатывает при успешном запросе и мы будет обновлять данные без обновления страницы
