...
export const useAuthForm = (isReg: boolean) => {
  ...
  // указываем типизацию
  // form это большой объект с методами работы с react-hook-form
  // <IAuthForm> Типизируется возвращаемое значение функции useForm
  ...
  const { mutate, isPending } = useMutation({
    // mutationKey — это уникальный ключ, который помогает React Query отличать одну мутацию от другой.
    // чтобы понимать, какие данные относятся к этой конкретной мутации;
    // чтобы правильно отслеживать состояние (loading, success, error) именно этой операции;
    // чтобы (при желании) можно было отменять, повторять или перезапускать конкретную мутацию по ключу.
